...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-plus"></i> 優待 / Modify </h1>
    </div>
@endsection
        ...
        <div class="col-md-12">
            <form name="changeform" method="POST">
                    ...
                    <div class="form-group @if($errors->has('vision_title')) has-error @endif">
                        <label for="vision_title-field">VISION タイトル</label>
                        <input type="text" id="vision_title-field" name="vision_title" class="form-control" value="{{ $vision_title }}" readonly="readonly"/>
                        @if($errors->has("vision_title"))
                           <span class="help-block">{{ $errors->first("vision_title") }}</span>
                        @endif
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group @if($errors->has('premier_title')) has-error @endif">
                        <label for="premier_title-field">タイトル</label>
                        <input type="text" id="premier_title-field" name="premier_title" class="form-control" value="{{ $premier_title }}"/>
                        @if($errors->has("premier_title"))
                           <span class="help-block">{{ $errors->first("premier_title") }}</span>
                        @endif
                    </div>
                    ...
                    <div class="form-group{{ $errors->has('start_dt') || $errors->has('start_dt_year') || $errors->has('start_dt_month') || $errors->has('start_dt_day') ? ' has-error' : '' }}">
                        <div class="row">
                            <div class="col-md-2" style="width:120px">
                                ...
                                <option value="">----</option>
                                @for ($i = 1980; $i <= 2025; $i++)
                                <option value="{{ $i }}"@if((old('start_dt_year') == $i) || ($start_dt_year == $i)) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                            ...
                            <div class="col-md-2" style="width:100px">
                                ...
                                <option value="">--</option>
                                @for ($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}"@if((old('start_dt_month') == $i) || ($start_dt_month == $i)) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                            ...
                            <div class="col-md-2" style="width:100px">
                                ...
                                <option value="">--</option>
                                @for ($i = 1; $i <= 31; $i++)
                                <option value="{{ $i }}"@if((old('start_dt_day') == $i) || ($start_dt_day == $i)) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                            ...
                            <div class="col-md-2" style="width:100px">
                                ...
                                <option value="">--</option>
                                @for ($i = 1; $i <= 24; $i++)
                                <option value="{{ $i }}"@if(old('start_dt_hour') or $start_dt_hour) == $i) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                            ...
                            <div class="col-md-2" style="width:100px">
                                ...
                                <option value="">--</option>
                                @for ($i = 0; $i <= 60; $i++)
                                <option value="{{ $i }}"@if((!is_null(old('start_dt_min')) && old('start_dt_min')) or $start_dt_min) == $i) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                            ...
                            <div class="col-md-2" style="width:100px">
                                ...
                                <option value="">--</option>
                                @for ($i = 0; $i <= 60; $i++)
                                <option value="{{ $i }}"@if((!is_null(old('start_dt_min')) && old('start_dt_sec')) or $start_dt_sec) == $i) selected @endif>{{ $i }}</option>
                                @endfor
                                </select>
                ...
                <div class="well well-sm">
                    <button type="button" class="btn btn-primary" onClick="E04_6_run()">確認2</button>
...
/* 新おみくじ登録確認画面 */
function E04_6_run() {
    window.document.changeform.action = "{{ url('connect/sell_detail_modify_confirm') }}";
